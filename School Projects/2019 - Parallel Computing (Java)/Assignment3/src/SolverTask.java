    ...
    public void findBestMoves() {
        ...
        for(long i=lo;i<hi;i++) {
            ...
            if (Flip.checkMoves(state, i)) {
                System.out.println("Correct I: "+Long.toBinaryString(i));
                ...
                do {
                    ...
                    if (!replaced && replacing) {
                        System.out.print("Failed to replace: "+Long.toBinaryString(res)+"-"+numMovesOld+" with: "+Long.toBinaryString(i)+"-"+numMovesNew);
                        res = result.get();
                        System.out.println(", it was: " + Long.toBinaryString(res));
                    } else if (replacing){
                        System.out.println("Successfully replaced: "+Long.toBinaryString(res)+"-"+numMovesOld+" with: "+Long.toBinaryString(i)+"-"+numMovesNew);
                        try {
                            SwingUtilities.invokeAndWait(new Runnable() {
                                @Override
                                public void run() {
                                    flip.updateSolution(result.get());
                                }
                            });
                        } catch (InterruptedException | InvocationTargetException e) { e.printStackTrace(); }
                    } else {
                        System.out.println("Successfully kept: "+Long.toBinaryString(res)+"-"+numMovesOld+", rejecting: "+Long.toBinaryString(i)+"-"+numMovesNew);
                    }
